...
def run_hover_switch(seed: int, tag: str, *, autotune: bool, init_alpha: float,
    ...
    best_hover_return = float("-inf")
    best_ckpt_path = os.path.join(LOG_DIR, f"{tag}_seed{seed}_best.pt")

    def _eval_now(step):
        nonlocal best_hover_return
        ...
        if out["return"] > best_hover_return:
            best_hover_return = out["return"]
            torch.save({
                "actor": agent.actor.state_dict(), "critic": agent.critic.state_dict(),
                "critic_target": agent.critic_target.state_dict(),
                "log_alpha": agent.log_alpha.detach().cpu(),
                "step": step, "best_return": best_hover_return,
...
def run_disc_sac(seed: int, tag: str, *, total_steps: int, eval_every: int, device=None):
    ...
    log = train_discrete_sac(env_fn, agent, total_steps=total_steps, eval_env_fn=env_fn,
                             eval_every=eval_every, eval_episodes=EVAL_EPISODES,
                             log_stdout=False, seed=seed, best_ckpt_fn=_save_best_disc)
    run_config = {
...
def run_dqn(seed: int, tag: str, *, total_steps: int, eval_every: int, device=None):
    ...
    log = train_dqn(env_fn, agent, total_steps=total_steps, eval_env_fn=env_fn,
                    eval_every=eval_every, eval_episodes=EVAL_EPISODES,
                    log_stdout=False, seed=seed, best_ckpt_fn=_save_best_dqn)
    run_config = {
